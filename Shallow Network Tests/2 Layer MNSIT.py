...
Original file is located at
    https://colab.research.google.com/drive/143Mm50KQWxTxq06Ca2DOglLLT_TJ80S9
"""
...
def get_error(scores, labels): #calculate error on network output given true labels
...
class two_layer_linear_net(nn.Module):

    def __init__(self, input_size, hidden_size1, output_size, weight_normalization = True):
    ...
    def forward(self, x, batch_normalization = True):
...
def eval_on_test_set(batch_norm = True): #evaluate network on testing set
...
plt.title("Error on MNIST Training Data 2 Layer Network")
plt.savefig("2layerTrain.png")
files.download("2layerTrain.png")
plt.show()
...
plt.title("Error on MNIST Test Data 2 Layer Network")
plt.savefig("2layerTest.png")
files.download("2layerTest.png")
plt.show()
